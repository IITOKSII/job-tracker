...
- **preview-engine.js downloadDoc():** Added `wrap.querySelectorAll(".tts-btn").forEach(btn => btn.remove())` after `wrap.innerHTML = previewEl.innerHTML` to strip TTS speaker buttons before html2canvas renders the PDF.
